...
import { X, GraduationCap, User, Phone, BookOpen, FileText, Briefcase, ChevronLeft, MapPin, Laptop, Home, Camera, Upload, CheckCircle2 } from "lucide-react";
...
export default function TeacherRegisterModal({ isOpen, onClose }: TeacherRegisterModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message = encodeURIComponent(
      ...
      (form.image ? `🖼️ *الصورة الشخصية:* سأقوم بإرسالها في الدردشة الآن\n` : "🖼️ *الصورة الشخصية:* لم يتم إرفاق صورة\n") +
  ...
  const handleClose = () => {
    onClose();
    setTimeout(() => {
      setForm({ name: "", phone: "", subject: "", stage: "", experience: "", bio: "", location: "", teachingMode: "", image: null, imagePreview: "" });
      setSubmitted(false);
              ...
              <div className="overflow-y-auto flex-1 overscroll-contain">
                <AnimatePresence mode="wait">
                  {!submitted ? (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center gap-3 pb-2">
                        <div className="relative group">
                          <div className={`w-24 h-24 rounded-2xl border-2 border-dashed ${form.imagePreview ? 'border-primary/50' : 'border-primary/20'} bg-primary/5 flex items-center justify-center overflow-hidden transition-all duration-300 group-hover:border-primary/40`}>
                            {form.imagePreview ? (
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (file) {
                                  ...
                                  reader.onloadend = () => {
                                    setForm({ ...form, image: file, imagePreview: reader.result as string });
                                  };
                          ...
                          <div className="absolute -bottom-2 -right-2 w-8 h-8 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg border-2 border-card group-hover:scale-110 transition-transform">
                            <Upload className="w-3.5 h-3.5" />
                          </div>
                        </div>
                        <div className="text-center">
                          <p className="text-xs font-bold text-foreground/80">الصورة الشخصية</p>
                          <p className="text-[10px] text-muted-foreground/60">يفضل صورة رسمية وبدقة عالية</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-2 bg-green-500/8 border border-green-500/20 rounded-xl px-4 py-3">
                        ...
                        <p className="text-[11px] text-green-400/80 leading-relaxed">
                          سيتم إرسال بياناتك عبر واتساب (يرجى إرسال الصورة في الدردشة أيضاً) وسنتواصل معك خلال <strong>24 ساعة</strong> لإتمام التسجيل.
                        </p>
                      </div>
                      ...
                      <Button
                        type="submit"
                        className="w-full bg-primary text-primary-foreground hover:bg-primary/90 rounded-xl py-5 text-sm font-black shadow-lg shadow-primary/25 cursor-pointer active:scale-[0.98] transition-all flex items-center justify-center gap-2 group"
                      >
                        <svg viewBox="0 0 24 24" className="w-4 h-4 transition-transform group-hover:scale-110" fill="currentColor">
                          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                        </svg>
                        إرسال عبر واتساب
                      </Button>
                    ...
                    >
                      <div className="w-20 h-20 rounded-full bg-green-500/15 border border-green-500/30 flex items-center justify-center shadow-[0_0_30px_rgba(34,197,94,0.15)]">
                      ...
                      <div>
                        ...
                        <m.div 
                          ...
                          className="mt-6 p-5 bg-primary/10 border border-primary/30 rounded-[2rem] flex flex-col items-center gap-4 relative overflow-hidden group"
                        >
                          ...
                          <div className="space-y-2">
                            <p className="text-sm font-black text-foreground leading-relaxed">
                              تذكير هام: يرجى إرسال صورتك الشخصية الآن
                            </p>
                            <p className="text-[11px] text-muted-foreground leading-relaxed px-4">
                              لقد قمنا بفتح محادثة الواتساب لك، من فضلك قم بـ <span className="text-primary font-bold">إرفاق الصورة الشخصية</span> في الدردشة لإكمال ملفك.
                            </p>
                          </div>
                        </m.div>

                        <p className="text-xs text-muted-foreground mt-8 opacity-60">
                          سنتواصل معك خلال <strong className="text-foreground">24 ساعة</strong> لمتابعة طلبك.
                        </p>
                      </div>
                      <Button
                        onClick={handleClose}
